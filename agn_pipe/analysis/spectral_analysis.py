...
class SpectralAnalysis:
    def __init__(
        ...
        if scratch_path is None:
            self.scratch_path = Path("./" + str(uuid.uuid4()))
        else:
    ...
    def setup_model(self, model_type="pwl", model_name=None):
    ...
    def get_spectral_points(self, e_min=0.1, e_max=30, n_bins=10):
    ...
    def run_lightcurve(self, duration, tstart=None, tstop=None):
        if tstart is None:
            tstart = self.tstart
        if tstop is None:
            tstop = self.tstop
